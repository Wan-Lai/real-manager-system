...
public class CustomerDao {
	...
	public static boolean addCustomer(Customer cus) throws Exception {
		String sql = "INSERT INTO customer(c_name, c_buy_address, c_address, c_phone, c_id_num, c_id_statu, c_time, c_want_type, c_commend)" + 
				"VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?);";
		int rst = sqlHelper.updateByPstmt(sql, cus.getName(), cus.getBuyaddress(), cus.getAddress(), cus.getPhone(), cus.getIdnum(), cus.getIdstatu(), cus.getTime(), cus.getWanttype(), cus.getCommend());
		return rst == 1;
	}
	...
	public static boolean updateCustomer(Customer cus) throws Exception {
		String sql = "UPDATE employee SET c_name=?, c_buy_address=?, c_address=?, c_phone=?, c_id_num=?, c_id_statu=?, c_time=?, c_want_type=?, c_commend=?" + 
				"WHERE e_id=?";
		int rst = sqlHelper.updateByPstmt(sql, cus.getName(), cus.getBuyaddress(), cus.getAddress(), cus.getPhone(), cus.getIdnum(), cus.getIdstatu(), cus.getTime(), cus.getWanttype(), cus.getCommend(), cus.getId());
		return rst == 1;
	}
