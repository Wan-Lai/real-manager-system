...
public class HouseDao {
	...
	public static boolean addHouse(House hou) throws Exception {
		...
		int rst = sqlHelper.updateByPstmt(sql, hou.getName(), hou.getNumber(), hou.getType(), hou.getPrice(),
				hou.getErea(), hou.getEid(), hou.getTime());
		return rst == 1;
	}
	...
	public static boolean updateHouse(House hou) throws Exception {
		String sql = "UPDATE house SET h_name=?, h_number=?, h_type=?, h_price=?, h_erea=?, e_id=?, h_time=? WHERE h_id=?";
		int rst = sqlHelper.updateByPstmt(sql, hou.getName(), hou.getNumber(), hou.getType(), hou.getPrice(),
				hou.getErea(), hou.getEid(), hou.getTime(), hou.getId());
		return rst == 1;
	}

	// 查询操作
	public static List<House> queryAllHouse() throws Exception {
		String sql = "SELECT h_id, h_name, h_number, h_type, h_price, h_erea, e_id, h_time FROM house";
